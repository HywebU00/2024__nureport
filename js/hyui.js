$(function () {
  ...
  //         sysHeight = Math.floor($('.sys_ann').outerHeight(true));
  ...
  function tabSet() {
    ...
    $('.tabs-2').each(function () {
      var _tab = $(this),
        _tabBtn = _tab.find('.tabBtn'),
        _tabContent = _tab.find('.tabContent'),
        tabwidth = _tab.width(),
        tabBtnHeight = _tabBtn.outerHeight(),
        tabContentHeight = _tab.find('.active').next().innerHeight(),
        tiGap = 0,
        tabBtnLength = _tabBtn.length,
        tabBtnWidth;
      _tab.find('.active').next('.tabContent').show();
      if (ww >= wwSmall) {
        _tabContent.css('top', tabBtnHeight);
        _tab.height(tabContentHeight + tabBtnHeight);
        tabBtnWidth = (tabwidth - (tabBtnLength - 1) * tiGap) / tabBtnLength;
        _tabBtn.width('auto').css('margin-left', tiGap);
        //_tabItem.first().css('margin-left', 0);
        //_tabItem.last().css({ 'position': 'absolute', 'top': 0, 'right': 0 }).width(tabItemWidth);
      } else {
        _tabBtn.width('auto').css('margin-right', tiGap);
        _tab.height('auto');
      }
      _tabBtn.focus(tabs); //改button後，前面改_tabItem
      _tabBtn.click(tabs); //改button後，前面改_tabItem
      function tabs(e) {
        var _tabBtnNow = $(this), //改button後，原來$(this).parent(),改$(this)
          tvp = _tab.offset().top,
          tabIndex = _tabBtnNow.index() / 2,
          scollDistance = tvp + tabBtnHeight * tabIndex - tab_headerHeight;
        _tabBtn.removeClass('active');
        _tabBtnNow.addClass('active');
        if (ww <= wwSmall) {
          _tabBtn.not('.active').next().slideUp();
          _tabBtnNow.next().slideDown();
          $('html,body').stop(true, false).animate({ scrollTop: scollDistance });
        } else {
          _tabBtn.not('.active').next().hide();
          _tabBtnNow.next().show();
          tabContentHeight = _tabBtnNow.next().innerHeight();
          _tab.height(tabContentHeight + tabBtnHeight);
        }
        e.preventDefault();
      }
  ...
  $('table').each(function (index, el) {
    //判斷沒有table_list
    if ($(this).parents('.table_list').length == 0 && $(this).parents('.fix_th_table').length == 0 && $(this).parent('form').length == 0 && $(this).parent('.table_normal').length == 0) {
      $(this).scroltable();
    }
  });
